...
@Testcontainers
@ActiveProfiles("test")
@Import(ScholarshipControllerIT.MailConfig.class)
class ScholarshipControllerIT {
  ...
  @Test
  void enforcesRoleDatesLifecycleVersionArchiveAndPublicVisibility() throws Exception {
    register();
    var id = users.findByEmailIgnoreCase("admin-scholarship@example.com").orElseThrow().getId();
   ...
   """))
    ...
    MvcResult periodResult =
        mvc.perform(
            ...
            .andReturn();
    String periodId =
        com.jayway.jsonpath.JsonPath.read(periodResult.getResponse().getContentAsString(), "$.id");
    MvcResult formResult =
        mvc.perform(
            ...
            .andReturn();
    String formId =
        com.jayway.jsonpath.JsonPath.read(formResult.getResponse().getContentAsString(), "$.id");
    mvc.perform(
            put("/api/admin/forms/" + formId + "/schema")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"version\":0,\"name\":\"Başvuru Formu\",\"sections\":[{\"title\":\"Genel\",\"description\":null,\"fields\":[{\"key\":\"motivation\",\"label\":\"Motivasyon\",\"type\":\"TEXTAREA\",\"required\":true,\"placeholder\":null,\"validationRules\":{},\"options\":[]}]}]}"))
    ...
    mvc.perform(
            put("/api/admin/scholarship-programs/" + programId)
                ...
                .cookie(access)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"version\":99,\"name\":\"X\",\"slug\":\"x\",\"description\":\"Y\"}"))
        .andExpect(status().isConflict())
  ...
  private void register() throws Exception {
    mvc.perform(
        post("/api/auth/register")
            ...
            .contentType(MediaType.APPLICATION_JSON)
            .content(
                "{\"email\":\"admin-scholarship@example.com\",\"password\":\"strong-pass-123\",\"firstName\":\"Admin\",\"lastName\":\"User\"}"));
    mvc.perform(
        post("/api/auth/verify-email")
            .with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content("{\"token\":\"" + mail.token.get() + "\"}"));
  }
  ...
  private Cookie login() throws Exception {
    return mvc.perform(
            post("/api/auth/login")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"email\":\"admin-scholarship@example.com\",\"password\":\"strong-pass-123\"}"))
        .andExpect(status().isOk())
        .andReturn()
        .getResponse()
  ...
  static class Mail implements AuthMailService {
    ...
    public void sendVerification(String e, String t) {
      token.set(t);
    }

    public void sendPasswordReset(String e, String t) {}
  }
}
