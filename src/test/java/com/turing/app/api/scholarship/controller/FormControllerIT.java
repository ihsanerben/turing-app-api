...
@Testcontainers
@ActiveProfiles("test")
@Import(FormControllerIT.MailConfig.class)
class FormControllerIT {
  ...
  void versionsFormSchemaAndKeepsPublishedVersionsImmutable() throws Exception {
    ...
    MvcResult copied =
        mvc.perform(
                post("/api/admin/forms/" + formId + "/new-version")
                    .with(csrf())
                    .cookie(access)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{\"version\":2}"))
            ...
            .andExpect(jsonPath("$.status").value("DRAFT"))
            .andExpect(jsonPath("$.sections[0].fields[0].key").value("family_income"))
            .andReturn();
    ...
    mvc.perform(
            put("/api/admin/forms/" + copyId + "/schema")
                .with(csrf())
                .cookie(access)
                .contentType(MediaType.APPLICATION_JSON)
                .content(schema.replace("\"version\":0", "\"version\":99")))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code").value("VERSION_CONFLICT"));
    mvc.perform(get("/api/admin/application-periods/" + periodId + "/forms").cookie(access))
        .andExpect(status().isOk())
  ...
  private Cookie adminSession() throws Exception {
    mvc.perform(
        post("/api/auth/register")
            ...
            .contentType(MediaType.APPLICATION_JSON)
            .content(
                "{\"email\":\"admin-form@example.com\",\"password\":\"strong-pass-123\",\"firstName\":\"Admin\",\"lastName\":\"Form\"}"));
    mvc.perform(
        post("/api/auth/verify-email")
            .with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content("{\"token\":\"" + mail.token.get() + "\"}"));
    var id = users.findByEmailIgnoreCase("admin-form@example.com").orElseThrow().getId();
    jdbc.update("update users set role='ADMIN' where id=?", id);
    return mvc.perform(
            post("/api/auth/login")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"email\":\"admin-form@example.com\",\"password\":\"strong-pass-123\"}"))
        .andExpect(status().isOk())
        .andReturn()
        .getResponse()
  ...
  static class Mail implements AuthMailService {
    ...
    public void sendVerification(String email, String value) {
      token.set(value);
    }

    public void sendPasswordReset(String email, String value) {}
  }
}
