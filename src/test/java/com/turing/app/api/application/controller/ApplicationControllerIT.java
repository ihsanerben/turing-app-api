...
@Testcontainers
@ActiveProfiles("test")
@Import(ApplicationControllerIT.MailConfig.class)
class ApplicationControllerIT {
  ...
  void protectsOwnershipValidatesTypedAnswersAndSubmitsAtomically() throws Exception {
    Cookie admin = session("application-admin@example.com");
    UUID adminId =
        users.findByEmailIgnoreCase("application-admin@example.com").orElseThrow().getId();
    jdbc.update("update users set role='ADMIN' where id=?", adminId);
    admin = sessionLogin("application-admin@example.com");
    Cookie student = session("application-student@example.com");
    createProfile(student);
    Cookie other = session("application-other@example.com");
    createProfile(other);
    ...
    mvc.perform(get("/api/me/applications/" + applicationId).cookie(other))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code").value("APPLICATION_NOT_FOUND"));

    MvcResult schema =
        mvc.perform(get("/api/me/applications/" + applicationId + "/form").cookie(student))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.form.versionNumber").value(1))
            .andReturn();
    List<String> fieldIds =
        JsonPath.read(schema.getResponse().getContentAsString(), "$.form.sections[0].fields[*].id");
    String motivationId = fieldIds.get(0),
        incomeId = fieldIds.get(1),
        housingId = fieldIds.get(2),
        skillsId = fieldIds.get(3);
    mvc.perform(
        ...
        .andExpect(jsonPath("$.code").value("REQUIRED_ANSWERS_MISSING"));
    assertThat(jdbc.queryForObject("select count(*) from application_snapshots", Integer.class))
        .isZero();

    String invalid =
        "{\"version\":0,\"answers\":[{\"fieldId\":\"" + housingId + "\",\"value\":\"invalid\"}]}";
    mvc.perform(
    ...
    mvc.perform(
            put("/api/me/applications/" + applicationId + "/answers")
                ...
                .content(answers))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.application.completion").value(100))
        .andExpect(jsonPath("$.application.version").value(1));
    assertThat(
            jdbc.queryForObject(
                "select count(*) from application_answers where text_value is not null",
                Integer.class))
        .isEqualTo(2);
    assertThat(
            jdbc.queryForObject(
                "select count(*) from application_answers where number_value is not null",
                Integer.class))
        .isEqualTo(1);
    assertThat(
            jdbc.queryForObject(
                "select count(*) from application_answers where json_value is not null",
                Integer.class))
        .isEqualTo(1);
    mvc.perform(
            put("/api/me/applications/" + applicationId + "/answers")
    ...
    assertThat(
        ...
        .isEqualTo(2);
    mvc.perform(
            post("/api/me/applications/" + applicationId + "/withdraw")
                .with(csrf())
                .cookie(student)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"version\":2}"))
    ...
    jdbc.update(
        "update application_periods set starts_at=now()-interval '2 days', ends_at=now()-interval '1 day' where id=?",
    ...
    mvc.perform(
            post("/api/me/applications")
                ...
                .content("{\"periodId\":\"" + periodId + "\"}"))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code").value("APPLICATION_PERIOD_CLOSED"));
  }

  @Test
  void adminListsFiltersNotesAndTransitionsApplicationsWithoutLeakingInternalsToStudent()
      throws Exception {
    Cookie admin = session("management-admin@example.com");
    UUID adminId =
        users.findByEmailIgnoreCase("management-admin@example.com").orElseThrow().getId();
    jdbc.update("update users set role='ADMIN' where id=?", adminId);
    admin = sessionLogin("management-admin@example.com");
    Cookie student = session("management-student@example.com");
    createProfile(student);
    String periodId = provisionPeriod(admin);
    ...
    List<String> ids =
        JsonPath.read(schema.getResponse().getContentAsString(), "$.form.sections[0].fields[*].id");
    String answers =
        "{\"version\":0,\"answers\":[{\"fieldId\":\""
            + ids.get(0)
            + "\",\"value\":\"Yeterince uzun motivasyon metni\"},{\"fieldId\":\""
            + ids.get(1)
            + "\",\"value\":1000},{\"fieldId\":\""
            + ids.get(2)
            + "\",\"value\":\"dorm\"}]}";
    ...
    mvc.perform(
            post("/api/me/applications/" + applicationId + "/submit")
                ...
                .content("{\"version\":1}"))
        .andExpect(status().isOk());
    mvc.perform(get("/api/admin/applications").cookie(student)).andExpect(status().isForbidden());
    mvc.perform(
            get("/api/admin/applications?search=management-student&status=SUBMITTED&sort=submittedAt&direction=asc")
                .cookie(admin))
        .andExpect(status().isOk())
  ...
  private String provisionPeriod(Cookie admin) throws Exception {
    String slug = "basari-bursu-" + UUID.randomUUID();
    String programId =
        JsonPath.read(
            mvc.perform(
                    post("/api/admin/scholarship-programs")
                        .with(csrf())
                        .cookie(admin)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(
                            "{\"name\":\"Başarı Bursu\",\"slug\":\""
                                + slug
                                + "\",\"description\":\"Öğrenci bursu\"}"))
                .andReturn()
                .getResponse()
                .getContentAsString(),
            "$.id");
    ...
    String periodId =
        JsonPath.read(
            mvc.perform(
                    post("/api/admin/application-periods")
                        .with(csrf())
                        .cookie(admin)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(
                            "{\"programId\":\""
                                + programId
                                + "\",\"name\":\"2026 Başvuruları\",\"academicYear\":\"2026-2027\",\"startsAt\":\""
                                + now.minusSeconds(60)
                                + "\",\"endsAt\":\""
                                + now.plusSeconds(86400)
                                + "\",\"maxRecipients\":20,\"allowWithdrawal\":true}"))
                .andReturn()
                .getResponse()
                .getContentAsString(),
            "$.id");
    String formId =
        JsonPath.read(
            mvc.perform(
                    post("/api/admin/application-periods/" + periodId + "/forms")
                        .with(csrf())
                        .cookie(admin)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"Başvuru Formu\"}"))
                .andReturn()
                .getResponse()
                .getContentAsString(),
            "$.id");
  ...
  private void createProfile(Cookie cookie) throws Exception {
    mvc.perform(
            put("/api/me/profile")
                .with(csrf())
                .cookie(cookie)
    ...
    mvc.perform(
            post("/api/auth/register")
                ...
                .content(
                    "{\"email\":\""
                        + email
                        + "\",\"password\":\"strong-pass-123\",\"firstName\":\"Test\",\"lastName\":\"Student\"}"))
        .andExpect(status().isCreated());
    mvc.perform(
        ...
        .andExpect(status().isOk());
    return sessionLogin(email);
  }

  private Cookie sessionLogin(String email) throws Exception {
    return mvc.perform(
            post("/api/auth/login")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
