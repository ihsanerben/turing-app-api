...
class ApplicationControllerIT {
    ...
    private String provisionPeriod(Cookie admin) throws Exception {String programId=JsonPath.read(mvc.perform(post("/api/admin/scholarship-programs").with(csrf()).cookie(admin).contentType(MediaType.APPLICATION_JSON).content("{\"name\":\"Başarı Bursu\",\"slug\":\"basari-bursu\",\"description\":\"Öğrenci bursu\"}")).andReturn().getResponse().getContentAsString(),"$.id");Instant now=Instant.now();String periodId=JsonPath.read(mvc.perform(post("/api/admin/application-periods").with(csrf()).cookie(admin).contentType(MediaType.APPLICATION_JSON).content("{\"programId\":\""+programId+"\",\"name\":\"2026 Başvuruları\",\"academicYear\":\"2026-2027\",\"startsAt\":\""+now.minusSeconds(60)+"\",\"endsAt\":\""+now.plusSeconds(86400)+"\",\"maxRecipients\":20,\"allowWithdrawal\":true}")).andReturn().getResponse().getContentAsString(),"$.id");String formId=JsonPath.read(mvc.perform(post("/api/admin/application-periods/"+periodId+"/forms").with(csrf()).cookie(admin).contentType(MediaType.APPLICATION_JSON).content("{\"name\":\"Başvuru Formu\"}")).andReturn().getResponse().getContentAsString(),"$.id");String formSchema="""
