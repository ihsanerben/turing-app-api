...
class ProfileControllerIT {
  ...
  void isolatesUsersEncryptsSensitiveDataAndAuditsAdminCorrection() throws Exception {
    ...
    mockMvc
        .perform(get("/api/me/profile").cookie(studentAccess))
  ...
  void rejectsDepartmentFromAnotherUniversity() throws Exception {
    registerAndVerify("mismatch@example.com");
    Cookie access = login("mismatch@example.com");
    mockMvc
        .perform(
            put("/api/me/profile")
                .with(csrf())
        ...
        .andExpect(jsonPath("$.code").value("DEPARTMENT_UNIVERSITY_MISMATCH"));
  }
