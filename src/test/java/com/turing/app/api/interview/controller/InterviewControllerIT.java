...
@Testcontainers
@ActiveProfiles("test")
@Import(InterviewControllerIT.MailConfig.class)
class InterviewControllerIT {
  ...
  @Test
  void protectsStudentViewAndManagesScheduleStatusFeedbackAndVersions() throws Exception {
    Cookie admin = admin("interview-admin@example.com"),
        student = session("interview-student@example.com"),
        other = session("interview-other@example.com");
    UUID
        studentId =
            users.findByEmailIgnoreCase("interview-student@example.com").orElseThrow().getId(),
        application = seed(studentId);
    String body =
        "{\"startsAt\":\"2026-09-03T10:00:00Z\",\"endsAt\":\"2026-09-03T11:00:00Z\",\"locationType\":\"ONLINE\",\"meetingUrl\":\"https://meet.example/interview\"}";
    String id =
        JsonPath.read(
            mvc.perform(
                    post("/api/admin/applications/" + application + "/interviews")
                        .with(csrf())
                        .cookie(admin)
    ...
    mvc.perform(get("/api/me/interviews/" + id).cookie(other)).andExpect(status().isNotFound());
    mvc.perform(
            patch("/api/admin/interviews/" + id + "/status")
                .with(csrf())
                .cookie(admin)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mvc.perform(
            put("/api/admin/interviews/" + id + "/feedback")
                .with(csrf())
                .cookie(admin)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"score\":85,\"notes\":\"Güçlü iletişim\",\"recommendation\":\"Olumlu\"}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.feedback[0].notes").value("Güçlü iletişim"));
    mvc.perform(get("/api/me/interviews/" + id).cookie(student))
        .andExpect(status().isOk())
  ...
  private UUID seed(UUID student) {
    ...
    jdbc.update(
        ...
        now);
    jdbc.update(
        "insert into applications(id,profile_id,period_id,form_id,status,completion,submitted_at,created_at,updated_at,version)values(?,?,?,?,'SHORTLISTED',100,?,?,?,0)",
        application,
        profile,
        period,
        form,
