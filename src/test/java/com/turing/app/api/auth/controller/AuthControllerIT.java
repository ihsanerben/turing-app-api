...
class AuthControllerIT {
    ...
    void completesRegistrationLoginRefreshAndDetectsReuse() throws Exception {
        mockMvc.perform(post("/api/auth/register").with(csrf()).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"email\":\"user@example.com\",\"password\":\"strong-pass-123\",\"firstName\":\"Ada\",\"lastName\":\"Lovelace\"}"))
        ...
        assertThat(access).isNotNull(); assertThat(access.isHttpOnly()).isTrue();
        ...
        MvcResult refresh = mockMvc.perform(post("/api/auth/refresh").with(csrf()).cookie(firstRefresh))
                .andExpect(status().isOk()).andReturn();
        Cookie secondRefresh = refresh.getResponse().getCookie("TURING_REFRESH_TOKEN");
        mockMvc.perform(post("/api/auth/refresh").with(csrf()).cookie(firstRefresh))
                .andExpect(status().isUnauthorized()).andExpect(jsonPath("$.code").value("REFRESH_TOKEN_REUSED"));
        mockMvc.perform(post("/api/auth/refresh").with(csrf()).cookie(secondRefresh))
    ...
    void rejectsStateChangingRequestWithoutCsrfToken() throws Exception {
        mockMvc.perform(post("/api/auth/register").contentType(MediaType.APPLICATION_JSON)
                .content("{\"email\":\"csrf@example.com\",\"password\":\"strong-pass-123\",\"firstName\":\"A\",\"lastName\":\"B\"}"))
                .andExpect(status().isForbidden());
    }

    @TestConfiguration
    static class TestMailConfig {
        @Bean @Primary CapturingMailService capturingMailService() { return new CapturingMailService(); }
    }
    static class CapturingMailService implements AuthMailService {
        ...
        public void sendVerification(String email, String token) { verification.set(token); }
        public void sendPasswordReset(String email, String token) { reset.set(token); }
    }
}
