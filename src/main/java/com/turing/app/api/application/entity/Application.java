...
public class Application {
  ...
  public static Application draft(
      StudentProfile profile, ApplicationPeriod period, FormDefinition form, Instant now) {
    Application value = new Application();
    value.id = UUID.randomUUID();
    value.profile = profile;
    value.period = period;
    value.form = form;
    value.status = ApplicationStatus.DRAFT;
    value.createdAt = now;
    value.updatedAt = now;
    return value;
  }

  public void answersChanged(int value, Instant now) {
    completion = value;
    updatedAt = now;
  }
