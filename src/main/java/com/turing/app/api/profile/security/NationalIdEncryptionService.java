...
public class NationalIdEncryptionService {
    ...
    public NationalIdEncryptionService(@Value("${app.profile.encryption-key}") String encodedKey) {
        ...
        if (decoded.length != 32) throw new IllegalStateException("PROFILE_ENCRYPTION_KEY must decode to 32 bytes");
    ...
    public byte[] encrypt(String value) {
        ...
        } catch (Exception exception) { throw new IllegalStateException("National ID encryption failed", exception); }
    }
    public String decrypt(byte[] value) {
        if (value == null) return null;
        try {
            ByteBuffer buffer = ByteBuffer.wrap(value); byte[] iv = new byte[IV_LENGTH]; buffer.get(iv);
            byte[] encrypted = new byte[buffer.remaining()]; buffer.get(encrypted);
            Cipher cipher = Cipher.getInstance("AES/GCM/NoPadding");
            cipher.init(Cipher.DECRYPT_MODE, key, new GCMParameterSpec(128, iv));
            return new String(cipher.doFinal(encrypted), StandardCharsets.UTF_8);
        } catch (Exception exception) { throw new IllegalStateException("National ID decryption failed", exception); }
    }
}
