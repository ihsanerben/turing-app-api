...
public class FormService {
    ...
    private final FormDefinitionRepository forms; private final ApplicationPeriodRepository periods;
    private final AuditService audit; private final ObjectMapper json; private final Clock clock;
    public FormService(FormDefinitionRepository forms,ApplicationPeriodRepository periods,AuditService audit,ObjectMapper json,Clock clock){this.forms=forms;this.periods=periods;this.audit=audit;this.json=json;this.clock=clock;}
    ...
    @Transactional
    public FormResponse saveSchema(UUID actor,UUID id,FormSchemaRequest request,String ip){FormDefinition form=findForm(id);ensureDraft(form);ensureConfigurable(form.getPeriod());checkVersion(form.getVersion(),request.version());validateSchema(request.sections());String before=snapshot(form);form.replaceSchema(request.name().trim(),toSections(request.sections()),clock.instant());forms.flush();audit.record(actor,"FORM_SCHEMA_UPDATED","FORM",id,before,snapshot(form),ip);return FormResponse.from(form);}
