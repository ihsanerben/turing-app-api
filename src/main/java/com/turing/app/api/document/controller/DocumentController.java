...
public class DocumentController {
  ...
  @GetMapping("/api/admin/application-periods/{periodId}/document-requirements")
  public List<DocumentRequirementResponse> adminRequirements(@PathVariable UUID periodId) {
    return service.adminRequirements(periodId);
  }

  @PostMapping("/api/admin/application-periods/{periodId}/document-requirements")
  @ResponseStatus(HttpStatus.CREATED)
  public DocumentRequirementResponse createRequirement(
      @PathVariable UUID periodId, @Valid @RequestBody DocumentRequirementRequest body) {
  ...
  public List<DocumentRequirementResponse> requirements(
    ...
    return service.applicationRequirements(user.id(), applicationId);
  }

  @GetMapping("/api/me/applications/{applicationId}/documents")
  public List<StoredFileResponse> documents(
      @AuthenticationPrincipal AuthenticatedUser user, @PathVariable UUID applicationId) {
    return service.list(user.id(), applicationId);
  }

  @PostMapping(
      value = "/api/me/applications/{applicationId}/documents",
  ...
  public StoredFileResponse upload(
    ...
    return service.upload(user.id(), applicationId, requirementId, file);
  }

  @GetMapping("/api/me/documents/{id}")
  public ResponseEntity<byte[]> download(
      @AuthenticationPrincipal AuthenticatedUser user, @PathVariable UUID id) {
    DocumentDownload value = service.download(user.id(), id);
