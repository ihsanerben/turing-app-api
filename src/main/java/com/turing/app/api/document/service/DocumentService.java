...
public class DocumentService {
  ...
  public List<DocumentRequirementResponse> adminRequirements(UUID periodId) {
    findPeriod(periodId);
    return requirements.findByPeriodIdOrderByDisplayOrderAsc(periodId).stream()
        .map(DocumentRequirementResponse::from)
        .toList();
  }

  @Transactional
  public DocumentRequirementResponse createRequirement(
      UUID periodId, DocumentRequirementRequest request) {
    ...
    try {
      return DocumentRequirementResponse.from(
      ...
      throw conflict("REQUIREMENT_CONFLICT", "Belge adı veya sırası bu dönemde kullanılıyor.");
    }
  }
  ...
  public List<StoredFileResponse> list(UUID userId, UUID applicationId) {
    ...
    return files
        .findByApplicationIdAndStatusOrderByRequirementDisplayOrderAsc(
            applicationId, FileStatus.ACTIVE)
        .stream()
  ...
  public StoredFileResponse upload(
      UUID userId, UUID applicationId, UUID requirementId, MultipartFile multipart) {
    Application app = findEditable(userId, applicationId);
    DocumentRequirement requirement =
        requirements
            .findById(requirementId)
            .orElseThrow(() -> notFound("REQUIREMENT_NOT_FOUND", "Belge gereksinimi bulunamadı."));
    if (!requirement.getPeriod().getId().equals(app.getPeriod().getId()))
      throw bad("REQUIREMENT_PERIOD_MISMATCH", "Belge gereksinimi bu başvuru dönemine ait değil.");
    byte[] content = bytes(multipart);
    String original = safeName(multipart.getOriginalFilename());
    String declared =
        Optional.ofNullable(multipart.getContentType()).orElse("").toLowerCase(Locale.ROOT);
    validateFile(requirement, original, declared, content);
    String key = "applications/" + applicationId + "/" + requirementId + "/" + UUID.randomUUID();
    String checksum = sha256(content);
    ObjectStorage storage = storage();
    storage.put(key, content, declared);
    try {
      files
          .findByApplicationIdAndRequirementIdAndStatus(
              applicationId, requirementId, FileStatus.ACTIVE)
          .ifPresent(
              previous -> {
                previous.replace(clock.instant());
                files.flush();
              });
      User owner =
          users
      ...
      compensateDelete(storage, key);
      if (exception instanceof ApplicationException applicationException)
        throw applicationException;
      throw new ApplicationException(
          HttpStatus.CONFLICT,
          "FILE_METADATA_FAILED",
          "Dosya kaydı tamamlanamadı. Tekrar deneyin.");
    }
  }

  @Transactional(readOnly = true)
  public DocumentDownload download(UUID userId, UUID fileId) {
  ...
  public void delete(UUID userId, UUID fileId) {
    ...
    if (file.getStatus() != FileStatus.ACTIVE)
      throw conflict("FILE_NOT_ACTIVE", "Yalnız aktif dosya silinebilir.");
    findEditable(userId, file.getApplication().getId());
  ...
  @Transactional(readOnly = true)
  public List<DocumentRequirementResponse> applicationRequirements(
      UUID userId, UUID applicationId) {
    Application app = findOwned(userId, applicationId);
    return requirements.findByPeriodIdOrderByDisplayOrderAsc(app.getPeriod().getId()).stream()
        .map(DocumentRequirementResponse::from)
        .toList();
  }
  ...
  private Application findEditable(UUID userId, UUID id) {
    ...
    if (app.getPeriod().getStatus() != PeriodStatus.OPEN
        ...
        || !now.isBefore(app.getPeriod().getEndsAt()))
  ...
  private StoredFile findOwnedFile(UUID userId, UUID id) {
    return files
        .findByIdAndOwnerId(id, userId)
        .orElseThrow(() -> notFound("FILE_NOT_FOUND", "Dosya bulunamadı."));
  }
  ...
  private ObjectStorage storage() {
    ...
    if (storage == null)
      throw new ApplicationException(
          HttpStatus.SERVICE_UNAVAILABLE, "STORAGE_UNAVAILABLE", "Dosya servisi kullanılamıyor.");
    return storage;
  }

  private byte[] bytes(MultipartFile file) {
    if (file == null || file.isEmpty()) throw bad("FILE_EMPTY", "Dosya boş olamaz.");
    try {
      return file.getBytes();
    } catch (Exception exception) {
      throw bad("FILE_READ_FAILED", "Dosya okunamadı.");
    }
  }

  private void validateFile(
      DocumentRequirement requirement, String name, String declared, byte[] content) {
    if (content.length > requirement.getMaxSizeBytes())
      throw new ApplicationException(
          HttpStatus.PAYLOAD_TOO_LARGE, "FILE_TOO_LARGE", "Dosya izin verilen boyutu aşıyor.");
    if (!requirement.getAllowedMimeTypes().contains(declared))
      throw new ApplicationException(
          HttpStatus.UNSUPPORTED_MEDIA_TYPE,
          "FILE_TYPE_NOT_ALLOWED",
          "Bu dosya türüne izin verilmiyor.");
    String detected = detect(content);
    if (!declared.equals(detected) || !extensionMatches(name, detected))
      throw new ApplicationException(
          HttpStatus.UNSUPPORTED_MEDIA_TYPE,
          "FILE_SIGNATURE_MISMATCH",
          "Dosya içeriği veya uzantısı bildirilen türle eşleşmiyor.");
  }
  ...
  private boolean extensionMatches(String name, String mime) {
    ...
    return mime.equals("application/pdf") && lower.endsWith(".pdf")
        || mime.equals("image/png") && lower.endsWith(".png")
        || mime.equals("image/jpeg") && (lower.endsWith(".jpg") || lower.endsWith(".jpeg"));
  }

  private String safeName(String value) {
    if (value == null || value.isBlank()) throw bad("INVALID_FILE_NAME", "Dosya adı geçersiz.");
    String name = value.replace('\\', '/');
    name = name.substring(name.lastIndexOf('/') + 1).trim();
    if (name.isBlank() || name.length() > 255 || name.chars().anyMatch(Character::isISOControl))
      throw bad("INVALID_FILE_NAME", "Dosya adı geçersiz.");
    return name;
  }
  ...
  private void compensateDelete(ObjectStorage storage, String key) {
    try {
      storage.delete(key);
    } catch (RuntimeException cleanup) {
      log.error("Upload compensation failed key={}", key, cleanup);
    }
  }
