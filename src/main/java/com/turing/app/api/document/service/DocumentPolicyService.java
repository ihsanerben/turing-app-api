...
public class DocumentPolicyService {
  ...
  public DocumentPolicyService(
      DocumentRequirementRepository requirements, StoredFileRepository files) {
    this.requirements = requirements;
    this.files = files;
  }

  @Transactional(readOnly = true)
  public List<String> missingRequired(Application app) {
    List<DocumentRequirement> required =
        requirements.findByPeriodIdOrderByDisplayOrderAsc(app.getPeriod().getId()).stream()
    ...
    Set<UUID> present =
        new HashSet<>(
            files
                ...
                .stream()
                .map(file -> file.getRequirement().getId())
                .toList());
    return required.stream()
        .filter(value -> !present.contains(value.getId()))
