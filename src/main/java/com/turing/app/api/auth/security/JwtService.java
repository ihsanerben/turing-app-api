...
public class JwtService {
  ...
  public String accessToken(User user, Instant now) {
    return encode(
        ...
        Map.of("type", "access", "email", user.getEmail(), "role", user.getRole().name()));
  }
  ...
  public String refreshToken(User user, UUID sessionId, Instant now) {
    return encode(
        ...
        Map.of("type", "refresh", "sid", sessionId.toString()));
  }

  private String encode(
      UUID subject, Instant issuedAt, Instant expiresAt, Map<String, Object> claims) {
